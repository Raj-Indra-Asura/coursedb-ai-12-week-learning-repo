...
async def list_topics(
    ...
    week_number: int | None = Query(None, ge=1, le=12, description="Filter by week number"),
...
async def get_topic(topic_id: int, db: Session = Depends(get_db)):
    ...
    if not topic:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Topic with id {topic_id} not found"
        )
